...
import { X, Activity, Zap, Wallet, Phone, Mail } from 'lucide-react';
...
const CustomerDetail = ({ customer, onClose }: CustomerDetailProps) => {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-2xl bg-slate-50 h-screen shadow-2xl flex flex-col"
        >
          {/* Header */}
            ...
            <div className="flex justify-between items-start relative z-10">
              <div className="flex items-center gap-5">
                ...
                <div>
                   ...
                   <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">ID: CID-{customer.id.substring(0, 8)}</p>
                </div>
              </div>
              <button onClick={onClose} className="p-3 bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-900 rounded-2xl transition-all">
                <X size={20} />
              </button>
            </div>
          </div>
          ...
          <main className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
            ...
            <div className="grid grid-cols-3 gap-6">
               ...
               <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-black text-amber-600">32-Dpd</p>
               </div>
               <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-black text-slate-900">₹{customer.amount.toLocaleString()}</p>
               </div>
            </div>
          ...
          <footer className="p-8 bg-white border-t border-slate-100 flex gap-4 shrink-0">
             <button className="flex-1 bg-slate-900 text-white rounded-2xl py-4 flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-900/20">
                <Phone size={16} /> 
                Intake Call
             </button>
             <button className="flex-1 bg-white text-slate-900 border-2 border-slate-100 rounded-2xl py-4 flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">
                <Mail size={16} /> 
                Dispatch Notice
             </button>
          </footer>
